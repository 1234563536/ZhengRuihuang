<template>
  <div class="register">
    ...
    <div class="main">
      ...
      <div class="login-box">
        <span class="pas"><a href="javascript:">更新密码</a></span>
        ...
        <div class="passenter reg-box">
          <form class="layui-form" id="form_updatepwd" method="post">
            <div class="layui-form-item username oldpassword">
              <label class="layui-form-label"><img
                  src="http://127.0.0.1:8002/loginpic/密码.png" alt=""></label>
            ...
            <div class="layui-form-item btn_register">
              ...
              <button class="layui-btn layui-btn-fluid layui-btn-normal" lay-submit lay-filter="updatepwdbtn"
              >注册
              </button>
            </div>
            <span class="btn_login"><a href="/login">登录</a></span>
...
export default {
  ...
  created() {
    ...
    window.layui.use(['jquery', 'layer'], function () {
      ...
      $('#form_updatepwd').on('submit', function (e) {
        e.preventDefault()
        return false
      });
      form.verify({
      ...
      form.on('submit(updatepwdbtn)', function () {
        if (window.localStorage.getItem("token") === "") {
          window.layer.msg("你还未登录，请先登录后，再修改密码")
          setTimeout(function () {
            location.href = "/login"
          }, 2000)
        } else {
          self.$axios({
            ...
            data: {oldPwd: self.oldPwd, newPwd: self.newPwd, id: parseInt(window.localStorage.getItem("userid"))},
          }).then(function (res) {
            console.log(res.data)
            if (res.data.status === 1) {
              window.layer.msg(res.data.message)
            } else {
              window.layer.msg("密码更新成功，请重新登录!")
              setTimeout(function () {
                location.href = "/login"
              }, 2000)
            }
